...
and syntax of a word, and is linked by means of a lexical sense to a ontology entity
...
that has the properties that the entity's type is a mountain, it is located in Australia,
and has an elevation bound to the variable \texttt{?elevation}. We then sort the
...
property and furthermore the arguments of each frame are linked to the sense, to
...
. A way to solve this bottleneck could be to introduce, for every possible subclass of {\tt Person}, default values; as well as to introduce default values for the same Artifact in conjunction with a predicate or adjective (e.\,g. {\tt BigPerson}, {\tt BulkyPerson}). 
