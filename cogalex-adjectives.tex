...
We consider how we can extend the ontology-lexicon interface as realized by extant models such as \emph{lemon} in the face of the issues mentioned above, in particular those arising in the context of modelling the ontological semantics of adjectives. We show how more complex logical formalisms required to capture the ontological semantics of adjectives can be backward 
...
adjectives are subsective, for example `a big mouse' is not `a big animal' \cite{morzycki2013nonscales}. \textbf{\textcolor{violet}{An 
important group of gradable adjectives are intersective}\textcolor{blue}{here we should better specificy that these adjectives are indeed call `absolute' gradable, although they are not naturally gradable.}}; we call 
...
\item There may be a minimum or maximum of this scale \textbf{(Which scale?)}, which can be determined
...
(e.g., RGB {\tt 0xff0000}).}\textcolor{blue}{We have already mentioned absolute gradable adjectives above; it still appears here that absolute gradable adjectives are still gradable, which are (per definition) not.}}
...
for the class of basketball players he is unusually short. \textbf{\textcolor{blue}{While SUMO has some very specific listings of subsets for the same \texttt{Artifact}\footnote{For example, the subsets for \texttt{Car} are \texttt{CrewDormCar}, \texttt{GalleryCar}, \texttt{MotorRailcar}, \texttt{FreightCar}, \texttt{BoxCar}, \texttt{RefrigeratorCar}, \texttt{FiveWellStackCar}, \texttt{FlatCar}, \texttt{SpineCar}, \texttt{HydraCushionFreightCar} and more.} for which it is then relatively simple to define maximum and minimum defaults for each for them and therefore consequently readjust the defaults for \texttt{Car} so to be the most prototypical possible, in some other places of its conceptual taxonomy, the ontology is still behind and needs further extension. For instance, with reference to this example of the basketball player, SUMO does not provide a well-structured subset net for \texttt{Person}, which is just enlisted as an `instance for \texttt{Human}'. Further classifications of \texttt{Person} (e.\,g. in terms of gender or age) are still missing.}}\textbf{As such we must introduce default values for
...
Operator adjectives are those that combine with a noun to alter the meaning of the noun itself. \textbf{\textcolor{blue}{Fran: not clear.}} 
...
people who were presidents. \textbf{Thus it can be said that $[\text{\em former}(\text{\em President})]$ creates a new class in the ontology. \textcolor{blue}{Just said.}} An alternative
...
\end{enumerate}
\textbf{Another example is `first', which has a context-dependent and highly ontology-specific meaning, 
often tightly interwoven with the meaning of the noun, as in the following examples \textcolor{blue}{This statement seems to repeat what we have reiterated so far for all (not only gradable adjectives), namely that they are context-dependent and they qualify nouns. What we can stress in the case of `first' is the fact that it acts the same as `former', since it automatically excludes the presidents that come after it. If so, the adjective should not be listed here under gradable adjectives, but be rather marked as operator adjective.}}:
\begin{enumerate}[resume]
\item \begin{enumerate}
...
Finally, there is only one instance of a non-empty subsective adjective, denoting a property:
\begin{enumerate}[resume]
...
a final note, we consider the claims that not all languages have adjectives
